...
/* Defines -------------------------------------------------------------------*/
#define HAL_ACC_SCK_PIN   17
#define HAL_ACC_MISO_PIN  14
#define HAL_ACC_MOSI_PIN  15
#define HAL_ACC_SS_PIN    13
#define HAL_ACC_INT_PIN   12
...
static void cs_high(void) {
  hal_pin_set_mode_ex(HAL_ACC_SS_PIN, HAL_PIN_MODE_OUT, HAL_PIN_MODE_EX_NORMAL, HAL_PIN_LVL_HIGH);
}
// Active CS
static void cs_low(void) {
  hal_pin_set_mode_ex(HAL_ACC_SS_PIN, HAL_PIN_MODE_OUT, HAL_PIN_MODE_EX_NORMAL, HAL_PIN_LVL_LOW);
}
// Read FIFO
static uint32_t read_fifo(uint8_t *buf, uint32_t len) {
  ...
  cs_low();
  xSPI->ops->write(xSPI, buf, 1);
  xSPI->ops->read(xSPI, buf, len * 2);
  cs_high();
...
static uint8_t read_reg(uint8_t addr) {
  ...
  cs_low();
  xSPI->ops->write(xSPI, buf, 2);
  xSPI->ops->read(xSPI, buf, 1);
  cs_high();
...
//  cs_low();
//  xSPI->ops->write(xSPI, tmp, 2);
//  xSPI->ops->write(xSPI, buf, len);
//  cs_high();
//}
  ...
  if(xSem != NULL && xSemaphoreTake(xSem, LOCK_TIMEOUT)) {
    ...
    if(xSPI != NULL) {
      hal_spi_cfg_t cfg = {
        ...
        .ss_pin = 0xff,
        .sck_pin = HAL_ACC_SCK_PIN,
        .miso_pin = HAL_ACC_MISO_PIN,
        .mosi_pin = HAL_ACC_MOSI_PIN,
      };
      xSPI->ops->init(xSPI, &cfg);
      return true;
    }
    xSemaphoreGive(xSem);
  }
