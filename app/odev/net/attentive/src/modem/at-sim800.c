...
static int sim800_socket_connect(struct cellular *modem, const char *host, uint16_t port)
{
    ...
    // TCP connection
        ...
        int n = 0;
        do {
            DBG_I("sim800_socket_connect: perform %d times, total retry times =%d",n,SIM800_TCP_CONNECT_RETRIES);
            at_set_timeout(modem->at, AT_TIMEOUT_SHORT);
            ...
            while(i++ < SIM800_TCP_CONNECT_TIMEOUT) {
                DBG_I("sim800_socket_connect: wait status %d times of total %d",i,SIM800_TCP_CONNECT_TIMEOUT);
                if (priv->socket_status[connid] == SIM800_SOCKET_STATUS_CONNECTED) {
                    return connid;
                } else if (priv->socket_status[connid] == SIM800_SOCKET_STATUS_ERROR) {
                    DBG_I("sim800_socket_connect: waiting connect ok");
                    if(sim800_ipstatus(modem) != 0) {
                        return -1;
